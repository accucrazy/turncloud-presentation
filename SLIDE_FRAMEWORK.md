...
### 結構（`rtx-talk-deck/index.html` 是唯一進入點，共 40 頁 = 封面 1 + CH01 12 + CH02 8 + CH03 19）

> **2026-07-23 大改版：整份 deck 重排、風格統一到暗色＋NVIDIA 綠（原 LOCAL 章風格）。** 外殼新增暗色封面 `slides/cover.html` 與三個章節過場 `slides/chapter-{nemo,pocket,local}.html`（CH01 綠／CH02 青／CH03 綠）。**CH01（原 NEMO PDF 截圖）全部重刻成暗色原生 HTML**；**CH02（POCKET）維持線上拉的精緻 Pocket 品牌淺色頁不動**（唯一保留原樣的一章，用章節過場銜接）；**CH03 加三面影片牆**。

| 段 | 來源陣列 | 型別 | 內容 |
| --- | --- | --- | --- |
| CH01 · NemoClaw × Nemotron | `NEMO`（1 過場 + 11 內容） | `html` → `slides/nemo-*.html` | **全暗色原生重刻**（原始 PDF 截圖仍留 `img/nemo/p01..p33`，只當內容參考／裁圖來源）：`chapter-nemo`(過場) `nemo-about`(p02 timeline 2017→2026，`.track` 橫軸) `nemo-clients`(p03 客戶牆，裁乾淨的 `p03_logos.png` 白卡) `nemo-team`(p04 agent 陣容，裁掉烤字標題的 `p04_team.png`) `nemo-finetune`(p06 Golden Dataset 三條件) `nemo-why-pandora`(p09 三家 LLM 只引 0~20 篇) `nemo-culture`(p14 Culture Listening 漏斗) `nemo-modelsize`(p16 30B=550B 四模型盲評) `nemo-task-matrix`(p18 任務×模型 2×2) `nemo-slm-paper`(p19 NVIDIA SLM 論文) `nemo-rtx5090`(p21 RTX 5090 · 93.5 · 手刻 terminal) `nemo-one-sentence`(p32 Telegram 一句話驅動)。**要加回被砍的 PDF 頁**：讀 `img/nemo/p*.png` 內容，比照既有 `nemo-*.html` 手法新做一頁，塞進 `NEMO` 陣列。 |
| CH02 · 騰雲發表會精選 | `chapter-pocket` + `POCKET`（7 張） | `html` 過場 + `pocket` | 唯一保留線上正式 deck 拆頁的一章：`p02_overview / p03_aios_concept / p05_harness_skill（VIN 伏筆）/ p06_pandora / p11_banana / p13_banana_video_edit / p18_ecosystem_chapter`，`srcdoc` + `<base href=POCKET_BASE>` 吃線上素材（淺色 Pocket 品牌風，刻意不動）。增減頁：從 `ai-talk-deck/pocket/` 複製 p*.html 進本 deck `pocket/` 再改 `POCKET` 陣列。 |
| CH03 · 地端多模態工作流 | `LOCAL`（1 過場 + 15 local + 3 牆） | `html` → `slides/local-*.html` / `slides/wall-*.html` | 開頭 `chapter-local` 過場。**新增三面影片牆（`slides/wall-*.html`，素材壓進 `img/walls/`，全 muted loop 自動播）**：`wall-reels`（4 支重製 IG reel `reel_recreated_with_music`，插在 lipsync 後）／`wall-ads`（beryl 四種情緒 seedance 廣告 noir/festive/sunlit/stillness，插在 vs-cloud 後）／`wall-drama`（武俠 5 集 `wuxia_ep1~5` 各截 8s taste，插在 agentic 後）。牆影片轉檔腳本 `_prep_walls.py`（parent repo 根，用 imageio-ffmpeg 完整 ffmpeg 壓 CRF27）。以下為原 15 頁 **單一敘事線**（大字、少字、故事驅動）——「內衣品牌被雲端擋 → VIN 伏筆回收 → Agent 逐位落地 → 可控實證 → 混合管線實證 → Content Tech」**：`local-intro`（故事開場：內衣品牌想出一批素材）→ `local-blocked`（雲端說 NO：**真實實測** 5/5 試穿圖輸入端被擋，`PrivacyInformation` / `OutputVideoSensitiveContentDetected`）→ `local-vin`（伏筆回收：TSpace 的腦可以換 —— Gemini/Claude ⇄ **Turncloud VIN**）→ `local-pipeline`（Agent 落地對照地圖：Pandora→VIN、Banana→ComfyUI）→ `local-pandora`（**Pandora 落地**：30B=550B、93.5、$0 API）→ `local-demo-a`（**Banana 落地 I**：單品照→實穿）→ `local-demo-b`（**Banana 落地 II**：實穿→影片，**真實影片** `tryon_video.mp4`）→ `local-control`（用求的 vs 用指揮的）→ `local-pose`（**指定舞步實證**：driver + Vivian/Beryl 跟跳，SDPose 骨架，3 支 muted loop 影片）→ **`local-lipsync`（跳舞＋對嘴同框：Wan 2.2 Animate 14B，driver 一次驅動身體＋手勢＋嘴型，19s 並排 `lipsync_sbs.mp4` muted loop；誠實成本：14B 溢出 24GB VRAM → offload 183s/步）**→ `local-vs-cloud`(**同一件旗袍實測對照**：地端 LTX-2.3 vs 雲端 Seedance r2v vs 雲端 editorial 廣告，結論=混合是正解) → **`local-drama`（混合實證 I：《重生之我不當棄婦》EP1 `douyin_ep1.mp4` 🔊controls，地端腦→Seedance ×6 平行→Qwen3-TTS＋FFmpeg 對位，端到端 ≈12 分/集）**→ **`local-agentic`（混合實證 II：《穿越者的資本江湖》五集完結 `wuxia_ep1.mp4` 🔊controls，SENSE→GATE→ACT→VERIFY→FIX→SHIP 控制迴圈、9 條自癒護欄、NT$133/集）**→ `local-sovereignty`（算力主權）→ `local-conclusion`（**Content Tech**）。影片素材來源：`video-package.zip`（driver_reel / pose_* / tryon_video / vs_*）＋ **`NVIDIA RTX workshop/ai-video-outputs-all/01-nearby-showcase/`**（lipsync_sbs / douyin_ep1 / wuxia_ep1，2026-07-23 複製進 `img/local/`）。 |
...
# 驗證 https://turncloud.thepocket.company/rtx-talk/?v=<隨機> → 40 頁、slides/*.html 200、pocket 段線上頁可載、img/walls/*.mp4 + img/local/*.mp4 200
```

---

## 16 · 衍生 deck · `tpc-siam-partnership`（泰國 Bureau of Wonders 合作提案 · 英文）
...
> **`index.html`（複製自 `tpc-launch-en` 再改）＋自帶 `img/`（Pandora 報告截圖 ×5 + Banana 生成曼谷版封面 + Accucrazy 自介圖 + 泰國實績圖 + SiamFoodie 錄屏 demo），共 33 頁**。共用素材仍吃線上根網域 `/img/`。
...
| 16（新增） | `siam_insight` | **4 個圖文洞察卡（左＝真實貼文截圖 30% 寬，右＝洞察）**：冰箱磁鐵 83,075 讚（souvenir economy）、芒果糯米 20K 讚吃播（craving 入口）、SÚNDALO 香水 $390（泰國在地品牌）、NEXTOPIA 永續 reel（「未來城市」）。每卡附 `→ 建議行動`。貼文截圖 `img/post_*.jpg` 是 **Instagram embed 頁**（`instagram.com/reel/<code>/embed/captioned/`，免登入）以 560 寬 viewport（dsf=1，dsf=2 會出現內容重複 bug）截的；貼文連結全部從 Pandora report-view 的文章列表 `a[href]` 挖出。**每張截圖已包 `<a target="_blank">` 連回原始 reel**（magnet `DWTqCE3CfIB` / mango `DWi1UDFCe4u` / SÚNDALO `DWOodmQEvHC` / NEXTOPIA `DTKWQgxicV-`），簡報中可直接點開 |
| 17（新增） | `siam_foodie` | **SiamFoodie live demo（洞察→產品的收尾）**：左＝手機直式錄屏 `img/siamfoodie_demo.mp4`（480×1040 · 8.88s · muted loop 自動播；內容＝選語言→English→捲 37 家餐廳牆→Cafe & Dessert 篩選→開 STARBUCKS 詳情菜單），右＝4 卡（5 語言 / AI concierge 非目錄 / booking→delivery 全流程 / built in days）。**錄製**：parent repo 根 `_record_siamfoodie.py`（Playwright headless 錄 webm）→ `_convert_sf.py` 轉檔（⚠️ Playwright 內建 ffmpeg 無 mp4 muxer，須用 `imageio-ffmpeg` 的完整 ffmpeg；掐頭 4.8s 載入等待）。⚠️ app 的 AI 對話後端會回 network error，腳本刻意不送訊息 · CSS `sp-foodie` |
| 31（新增） | `siam_framework` | **Content Tech flywheel**：LISTEN(Pandora)→CREATE(Moana×Banana)→ACTIVATE(Adriana×Stacey)→COMPOUND(自有 intelligence asset)＋三條合作軌（Destination Intelligence / Content Engine / Ecosystem Play）＋ 90-day pilot 收尾條 |
| 33 redeem | `redeem` | 對象改成 Siam Piwat team（footnote 同步改） |
...
# 驗證 https://turncloud.thepocket.company/siam-partnership/?v=<隨機> → 33 頁、../img/ 素材全 200
```
